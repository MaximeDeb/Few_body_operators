...
class classMPO:
    ...
    def rotate_Herm(self, A):
        active = np.arange(L)
        for i in range(self.L):
            l = self.L - i
            sect = active[:l]

            Aev, Aevecs = np.linalg.eigh(A[:l,:l])        ## Compute the correlation matrix spectrum in the current set of orbitals. Remove the orbital that was just rotated from Q
            # order = np.argsort(Rev*(1-Rev))[::-1]       ## Sort in decreasing order from 1/2: last orbital has occupation the closest to 0 or 1
            order = np.argsort(0.5-np.abs(Aev))[::-1]     ## Sort in decreasing order from 1/2: last orbital has occupation the closest to 0 or 1
    ...
    def rotate_Unit(self, R):
        ...
        for i in range(self.L):
            ...
            U, S, Vd = np.linalg.svd(A[:l,:l])            ## Compute the correlation matrix spectrum in the current set of orbitals. Remove the orbital that was just rotated from Q
            
            order = np.argsort(0.5-np.abs(S))[::-1]     ## Sort in decreasing order from 1/2: last orbital has occupation the closest to 0 or 1
            S, Lev, Rev = S[order], U[:,order], Vd.conj().T[:,order]

            ## Apply givens rotations to localize the last orbital and put it to the right of the active part. Stack orbitals from right to left with occupancies getting closer to 1/2.
            indices, Lgivens = Givens_rotations(Lev, [l-1], sect, direction="right")  
            indices, Rgivens = Givens_rotations(Rev.conj(), [l-1], sect, direction="right")   ## L and R indices are the same

            for m, ind in enumerate(indices):
                Lgate_giv, Rgate_giv = np.eye(4, dtype=complex), np.eye(4, dtype=complex) ## Transforms the givens rotation in a quantum gate
